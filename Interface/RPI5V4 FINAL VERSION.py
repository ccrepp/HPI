#!/usr/bin/env python3
"""
Smart Plant Monitoring System - Fully Integrated Version
- On-demand sensor reading via touch/mouse interface
- Default "Normal" state animation
- Video playback directly integrated into PyGame
...
# Video paths
VIDEOS = {
    "normal": "/home/UNM.RPI5/videos/normal.mp4",    # Default state
    "dry": "/home/UNM.RPI5/videos/dry.mp4",          # Dry soil state
    "wet": "/home/UNM.RPI5/videos/wet.mp4",          # Wet soil state
    "dark": "/home/UNM.RPI5/videos/dark.mp4",        # Low light state
    "bright": "/home/UNM.RPI5/videos/bright.mp4",    # Bright light state
    "hot": "/home/UNM.RPI5/videos/hot.mp4",          # High temperature state
    "cold": "/home/UNM.RPI5/videos/cold.mp4",        # Low temperature state
    "humid": "/home/UNM.RPI5/videos/humid.mp4"       # High humidity state
}
...
SENSOR_THRESHOLDS = {
    ...
    "light": {
        "dark": 220,    # Below this value is dark
        "bright": 130  # Above this value is bright
    },
    "temperature": {
...
current_video_until = None  # Time until which to show current video before returning to normal
...
def load_video(video_key, duration=None):
    """Load and prepare a video for playback"""
    global current_video, current_video_path, video_capture, video_frame_count, video_fps, current_video_until
    ...
    loading_surface = pygame.Surface((SCREEN_WIDTH, int(SCREEN_HEIGHT * 0.7)))
    ...
    try:
        ...
        print(f"Loaded video: {video_path}, {video_frame_count} frames, {video_fps} FPS")
        
        # Set duration timer if specified
        if duration:
            current_video_until = time.time() + duration
        else:
            current_video_until = None
...
def render_current_frame():
    """Render the current video frame to the screen"""
    global video_capture, video_frame_count, current_video, current_video_until
    
    # Check if we need to switch back to normal video
    if current_video_until and time.time() > current_video_until:
        load_video("normal")
        current_video_until = None
    
    # Check if video is loaded
    if video_capture is None or not video_capture.isOpened():
        if current_video_path:  # Try to reload if path exists
            load_video(current_video)
        return
    
    # Calculate video area (top 70% of screen)
    video_area_height = int(SCREEN_HEIGHT * 0.7)
    video_area_width = int(SCREEN_WIDTH * 0.7) 
    video_x = (SCREEN_WIDTH - video_area_width) // 2
    video_y = int(SCREEN_HEIGHT * 0.05)
    video_area = pygame.Rect(video_x, video_y, video_area_width, video_area_height)
    
    # Read the next frame
    ret, frame = video_capture.read()
    
    # If frame read was not successful, reset to beginning of video (loop)
    if not ret:
        video_capture.set(cv2.CAP_PROP_POS_FRAMES, 0)
        ret, frame = video_capture.read()
        if not ret:  # Still failed, there's a problem with the video
            print(f"Error: Could not read frame from {current_video_path}")
            return
    
    # Resize the frame to fit the video area
    frame = cv2.resize(frame, (video_area.width, video_area.height))
    ...
    # Display the frame in the video area
    screen.blit(surface, (video_area.x, video_area.y))
...
def display_osd_value(sensor_type, value, status="normal"):
    ...
    if status != "normal" and sensor_type in ["water", "light", "temperature", "humidity"]:
        ...
        if status_video:
            load_video(status_video, duration=5)  # Play for 5 seconds then restore normal
...
def draw_ui(buttons, button_states):
    ...
    # Draw video area background (in case no video is playing)
    video_area_height = int(SCREEN_HEIGHT * 0.7)
    video_area_width = int(SCREEN_WIDTH * 0.7)
    video_x = (SCREEN_WIDTH - video_area_width) // 2
    video_y = int(SCREEN_HEIGHT * 0.05)
    video_area = pygame.Rect(video_x, video_y, video_area_width, video_area_height)
    pygame.draw.rect(screen, (0, 0, 0), video_area)
    
    # Render current video frame in the top 70% of the screen
    render_current_frame()
    
    # Draw divider line between video and controls
    pygame.draw.line(screen, (100, 100, 100), (0, video_area_height), 
                     (SCREEN_WIDTH, video_area_height), 2)
    
    # Fill the control area with a dark background
    control_area = pygame.Rect(0, video_area_height, SCREEN_WIDTH, SCREEN_HEIGHT - video_area_height)
    pygame.draw.rect(screen, (20, 20, 20), control_area)
...
def main():
    ...
    # Load default video
    if not load_video("normal"):
        print("Failed to load default video. Check if OpenCV is installed properly.")
        # Continue anyway, will show black background
    
    # Create button definitions with relative positioning
    buttons = [
        {"text": "Moisture", "rect": pygame.Rect(int(SCREEN_WIDTH * 0.05), int(SCREEN_HEIGHT * 0.85), 
                                             int(SCREEN_WIDTH * 0.2), int(SCREEN_HEIGHT * 0.08)), "action": "water"},
        {"text": "Light", "rect": pygame.Rect(int(SCREEN_WIDTH * 0.3), int(SCREEN_HEIGHT * 0.85), 
                                          int(SCREEN_WIDTH * 0.2), int(SCREEN_HEIGHT * 0.08)), "action": "light"},
        {"text": "Temp/Humid", "rect": pygame.Rect(int(SCREEN_WIDTH * 0.55), int(SCREEN_HEIGHT * 0.85), 
                                               int(SCREEN_WIDTH * 0.2), int(SCREEN_HEIGHT * 0.08)), "action": "dht"},
        {"text": "Reset", "rect": pygame.Rect(int(SCREEN_WIDTH * 0.8), int(SCREEN_HEIGHT * 0.85), 
                                          int(SCREEN_WIDTH * 0.15), int(SCREEN_HEIGHT * 0.08)), "action": "reset"}
    ]
