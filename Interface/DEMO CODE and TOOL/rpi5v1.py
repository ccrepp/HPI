...
def play_video(video_key, duration=None):
    """Play the specified video, with optional duration"""
    global video_process, current_video
    
    # Update current video state regardless of file existence
    ...
    cmd = [
        ...
        "--fs",
        "--no-input-terminal",  # Disable terminal input to avoid interference
        "--really-quiet",       # Reduce output
        "--no-osc",             # Disable on-screen controls
        video_path
    ]
    
    # Set environment variables to ensure correct display
